...
- Lock is an absolute geometry invariant, including mixed grouped selections. Exclude locked objects from drag, resize, rotation, snapping bounds, live Konva transforms, and committed batch geometry updates; build the Transformer only from selected unlocked objects. Locked members may remain visibly selected for context but must not move or change dimensions/rotation.
- Use a solid cyan/blue stroke for persistent object-selection bounds and Transformer borders. Reserve dashed strokes for transient marquee selection, crop mode, safe-area guides, and other non-object guides so selection state reads cleanly in the modern Adobe-style hierarchy.
